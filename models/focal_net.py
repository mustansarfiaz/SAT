...
from models.maxvit import MaxViTTransformerBlock, grid_partition, grid_reverse, MBConv, window_partition,window_reverse 
from models.ms_mixing import MixShiftBlock
...
class FocalModulation(nn.Module):
    def __init__(self, dim, focal_window, focal_level, focal_factor=2, bias=True, proj_drop=0., use_postln=False):
        ...
        self.f = nn.Linear(dim, 2*dim , bias=bias)
        ...
        for k in range(self.focal_level):
            ...
            self.ms_layers.append(MixShiftBlock(dim, 196, shift_size=3, shift_dist=[-1,1,0], mix_size= [1,3,5]))                       
    ...
    def forward(self, x):
        ...
        q, ctx = torch.split(x, (C, C), 1)
        
        
        # context aggreation
        ctx_all = 0 
        for l in range(self.focal_level):            
            f_ctx = self.focal_layers[l](ctx)  
            ctx_ms = self.ms_layers[l](f_ctx)
            ctx_all = ctx_all + ctx_ms
        ctx_global = self.act(f_ctx.mean(2, keepdim=True).mean(3, keepdim=True))
        ctx_all = ctx_all + ctx_global

        # focal modulation
        self.modulator = self.h(ctx_all)
        x_out = torch.cat((q,self.modulator),1)
